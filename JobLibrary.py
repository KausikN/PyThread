...
def JobInit_Random(JobFunc, size=[1, 1], n_operands=2, operand_type=int, valRange=[-100, 100]):
    ...
    if operand_type is None:
        GeneratedVals = np.array([valRange] * flatSize)
...
def Job_GenerateRandom_Looped(*operands, valRange):
...
def Job_GenerateRandom_Numpy(*operands, valRange):
    genSize = np.array(operands)
    genRand = np.random.randint(valRange[0], valRange[1], genSize)
    return genRand

def Job_Wait(seconds):
    time.sleep(seconds)
    return None
